...
- GIVEN the pre-existing Novel Reader settings screen had nine groups (Gemini, Google Translate, Display, E-Ink refresh, Theme, Navigation, Accessibility, TTS, Dictionary, Advanced)
...
- WHEN the tab renders
- THEN it contains the data group (database clearing), reader cache/decode settings, and the data saver group
...
- THEN they reach the same `SettingsTrackingScreen` that Configurações → Conexões → Rastreamento leads to, and both paths remain valid indefinitely
...
Source: navigation audit performed against `SettingsSearchScreen.kt:485-509` during planning.

#### Scenario: Every pre-existing Settings screen has exactly one home
- GIVEN the 10 pre-existing Settings screens (Appearance, Library, Reader, Novel Reader, Downloads, Tracking, Browse, Data, Security, Advanced) and the 10 Player Settings sub-screens
- WHEN the new 5-domain structure is in place
- THEN each of the 20 screens is reachable by exactly one navigation path from the More tab (excluding the deliberately-kept tracking shortcut)

#### Scenario: Every non-Settings destination retains its path
- GIVEN Categorias, Estatísticas, Conquistas, Tesouraria, Fila de downloads, Erros de atualização, and Ajuda were reachable from the More tab before this change
- WHEN the new structure ships
- THEN all seven remain reachable directly from the More tab root

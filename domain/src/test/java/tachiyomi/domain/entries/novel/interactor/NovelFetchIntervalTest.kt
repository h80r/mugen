...
class NovelFetchIntervalTest {
    ...
    private var chapter = NovelChapter.create().copy(
        dateFetch = testTime.toEpochSecond() * 1000,
        dateUpload = testTime.toEpochSecond() * 1000,
    )

    private val fetchInterval = NovelFetchInterval(mockk())
    ...
    private fun chapterWithTime(chapter: NovelChapter, duration: Duration): NovelChapter {
    ...
    private fun List<NovelChapter>.lastUploadDate() =

...
public class PageCurlState(
    ...
    /**
     * Moves to [value] without suspending, for callers that must stay in step within the frame they are already
     * composing — [snapTo] can only run from a coroutine, so its effect lands a frame late and the page visibly
     * flashes its previous content first.
     *
     * Only adjusts [current]; unlike [snapTo] it does not reset the fold edges, so call it while no fold is in
     * flight (progress == 0f). Added for the two-page spread, where the two independent PageCurl instances have to
     * show the same spread on the same frame.
     */
    public fun setCurrentImmediately(value: Int) {
        current = value.coerceIn(0, max - 1)
    }
...
// Names kept as upstream's to keep this vendored file diffable against pagecurl 1.5.1.
